...
public class MergeTest6 {
    ...
    public void testViewInconsistency() throws Exception {
        ...
        stack.insertProtocol(drop_view, ProtocolStack.BELOW, NAKACK2.class); // GMS.class);
        ...
        for(JChannel ch: Arrays.asList(three,four,five,six,seven))
            System.out.printf("%s: %s\n", ch.getAddress(), ch.getView());
    ...
    protected static JChannel createChannel(final int num) throws Exception {
        ...
        ch.addAddressGenerator(new AddressGenerator() {
            public Address generateAddress() {return new UUID(0, num);}
        });
        ch.connect("MergeTest6");
        return ch;
    }
    ...
    protected static class DropView extends Protocol {
        ...
        public Object up(Event evt) {
            if(evt.getType() == Event.MSG) {
                ...
                GMS.GmsHeader hdr=(GMS.GmsHeader)msg.getHeader(GMS_ID); View view;
